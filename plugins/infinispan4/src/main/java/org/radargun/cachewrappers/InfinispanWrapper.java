...
//          }
        ...
        if (cache.getCacheConfiguration().clustering().cacheMode().isDistributed() && !cache.getAdvancedCache().getDistributionManager().isJoinComplete())
    ...
    private boolean isPassiveReplication() {
        return cache.getCacheConfiguration().transaction().transactionProtocol().isPassiveReplication();
    }
