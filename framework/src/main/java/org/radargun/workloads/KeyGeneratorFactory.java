...
public class KeyGeneratorFactory {
   ...
   public Iterator<PopulationEntry> populateNode(int nodeIdx) {
      return new InternalNodeIterator(nodeIdx, currentWorkload.get());
   }

   public Iterator<PopulationEntry> populateAll() {
   ...
   protected Object getRandomValue(Random random) {
   ...
   protected Object createKey(int nodeIdx, int threadIdx, int keyIdx) {
   ...
   private class InternalNodeIterator implements Iterator<PopulationEntry> {
      ...
      @Override
      public PopulationEntry next() {
         if (!hasNext()) {
            throw new NoSuchElementException();
         }
         PopulationEntry next = new PopulationEntry(getBucket(threadIdx),
