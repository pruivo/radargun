...
public class TPCCPopulation {
   ...
   private void populate_district(int id_warehouse) {
      if (id_warehouse < 0) return;
      else {
         ...
         for (int id_district = init_id_district; id_district <= (init_id_district-1+num_of_districts); id_district++) {
            ...
            populate_customer(id_warehouse, id_district);
            populate_order(id_warehouse, id_district);
         }
      }
   }
   ...
   protected void populate_customer(int id_warehouse, int id_district) {
      if (id_warehouse < 0 || id_district < 0) return;
         ...
         for (int i = 1; i <= TPCCTools.NB_MAX_CUSTOMER; i++) {
            ...
            populate_history(i, id_warehouse, id_district);
         }
      }
   }

   //Not to be parallelized
   protected void populate_history(int id_customer, int id_wharehouse, int id_district) {
   ...
   protected void populate_order(int id_warehouse, int id_district) {
      ...
      for (int id_order = 1; id_order <= TPCCTools.NB_MAX_ORDER; id_order++) {
         ...
         populate_order_line(id_warehouse, id_district, id_order, o_ol_cnt, aDate);

         if (id_order >= TPCCTools.LIMIT_ORDER)
            populate_new_order(id_warehouse, id_district, id_order);
      }
   }

   protected void populate_order_line(int id_warehouse, int id_district, int id_order, int o_ol_cnt, Date aDate) {
   ...
   protected void populate_new_order(int id_warehouse, int id_district, int id_order) {
   ...
   protected int generate_seq_alea(int deb, int fin) {
      if (!this._new_order) {
