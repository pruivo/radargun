...
public class TPCCTools {
   ...
   public static List<Integer> selectLocalWarehouse(CacheWrapper wrapper, int numberOfSlaves, int slaveIdx) {


      if(wrapper.isFullyReplicated()){
         return Collections.emptyList();
      }	else{
         List<Integer> result = new ArrayList<Integer>((TPCCPopulation.NB_WAREHOUSE / numberOfSlaves) + 1);
         int init = slaveIdx % TPCCPopulation.NB_WAREHOUSE;         
         
         for (int i = init + 1; i <= TPCCPopulation.NB_WAREHOUSE; i += numberOfSlaves) {
            result.add(i);
         }
