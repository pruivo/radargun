...
public class PutGetStressor implements CacheWrapperStressor {
   ...
   private ResultBean executeOperations() throws Exception {
      ...
      List<Integer> listLocalWarehouses = TPCCTools.selectLocalWarehouse(cacheWrapper,total_num_of_slaves, slaveIndex);
   ...
   private class Stresser extends Thread {
      ...
      public Stresser(int localWarehouse, int threadIndex, AtomicInteger requestsLeft, boolean is_master_thread, 
                      int lb, int ub, long time, long c_run, long c_id, long ol_i_id, boolean forceGlobalWrite) {
         ...
         log.debug(getName() + " will stress warehouse " + localWarehouse);
      }
