...
import org.radargun.workloads.RadargunKey;
...
public class KeyGeneratorTest {
   ...
   public void testSingleMastWarmup() {
      ...
      Iterator<PopulationEntry> iterator = factory.populateAll();
   ...
   public void testMultiMasterWarmup() {
      ...
      for (int nodeIdx = 0; nodeIdx < numberOfNodes; ++nodeIdx) {
         Iterator<PopulationEntry> iterator = factory.populateNode(nodeIdx);
         ...
         for (int numberOfThreads = 1; numberOfThreads < 8; ++numberOfThreads) {
            for (int numberOfKeys = 1000; numberOfKeys < 10000000; numberOfKeys *= 10) {
               ...
               while (iterator.hasNext()) {
                  PopulationEntry entry = iterator.next();
   ...
   private void assertIterator(Iterator<PopulationEntry> iterator, int nodeIdx, int threadIdx, int maxKeys) {
      for (int i = 0; i < maxKeys; ++i) {
